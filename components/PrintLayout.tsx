...
const PrintLayout: React.FC<PrintLayoutProps> = ({ program, quote, onBack, customLogo, customBanner, timetableImages, portalType, onUploadLogo, onSaveLogoDefault }) => {
    ...
    return (
      ...
      */}
            <div id="printable-content" className="pdf-container">
                <div className="w-[210mm] h-[297mm] max-w-[210mm] max-h-[297mm] mx-auto bg-white shadow-2xl print:shadow-none relative overflow-hidden flex flex-col mt-8 print:mt-0" style={fontStyle}>
                    ...
                    {displayBanner ? (
                        <div className="w-full h-32 md:h-40 print:h-16 shrink-0 overflow-hidden">
                            <img src={displayBanner} alt="Header Banner" className="w-full h-full object-cover" />
                        </div>
                    ) : (
                        <div className={`h-4 print:h-2 w-full shrink-0 ${BSC_GRADIENT}`}></div>
                    )}

                    <div className="p-12 print:p-3 flex-grow flex flex-col print:text-[9px]">
                        {/* Header */}
                        <header className="flex justify-between items-start mb-8 print:mb-2 print:hidden">
                        ...
                        <div className="mb-8 print:hidden relative rounded-2xl overflow-hidden h-60 shrink-0">
                        ...
                        <div className="hidden print:flex justify-between items-start mb-2">
                            <div>
                                <h1 className="text-base font-bold text-slate-800">{program.name}</h1>
                                <div className="flex items-center text-gray-600 gap-1 text-[8px]">
                                    <MapPin className="w-2 h-2" />
                                    <span>{program.city}, {program.country}</span>
                                    <span className="mx-1">•</span>
                                    <CalendarDays className="w-2 h-2" />
                                    <span>{quote.durationWeeks} Hafta</span>
                                    <span className="mx-1">•</span>
                            ...
                            <div className="text-right text-[8px]">
                                <div className="font-bold text-gray-900">{quote.consultantName}</div>
                                <div className="text-gray-600">{quote.agencyName}</div>
                            </div>
                        </div>

                        {/* Content Grid */}
                        <div className="grid grid-cols-2 gap-10 print:gap-3 mb-8 print:mb-2 flex-grow">
                            {/* Left Column: Program Details & Services */}
                            <div className="space-y-6 print:space-y-2">
                                <div>
                                    <h3 className={`text-lg print:text-[10px] font-bold border-b-2 print:border-b pb-2 print:pb-1 mb-4 print:mb-1 uppercase tracking-wide ${theme.textDark} ${theme.borderLight}`}>Program Detayları</h3>
                                    <ul className="space-y-4 print:space-y-1">
                                        <li className={`flex items-start gap-4 print:gap-2 p-3 print:p-1 rounded-lg print:rounded ${theme.bgLight}`}>
                                ...
                                {/* Included Services */}
                                <div>
                                    <h3 className={`text-lg print:text-[10px] font-bold border-b-2 print:border-b pb-2 print:pb-1 mb-4 print:mb-1 uppercase tracking-wide ${theme.textDark} ${theme.borderLight}`}>Dahil Hizmetler</h3>
                                    <ul className="space-y-2 print:space-y-0.5">
                                        {program.includedServices.map((service, idx) => (
                                            <li key={idx} className="text-sm print:text-[8px] text-gray-700 flex items-start gap-2.5 print:gap-1">
                                                <Check className="w-4 h-4 print:w-2 print:h-2 mt-0.5 shrink-0 text-[#6499E9]" />
                                ...
                                {program.youngLearnersGoals && program.youngLearnersGoals.length > 0 && (
                                    <div>
                                        <h3 className={`text-lg print:text-[10px] font-bold border-b-2 print:border-b pb-2 print:pb-1 mb-4 print:mb-1 uppercase tracking-wide ${theme.textDark} ${theme.borderLight}`}>Kazanımlar</h3>
                                        <ul className="space-y-2 print:space-y-0.5">
                                            {program.youngLearnersGoals.map((goal, idx) => (
                                                <li key={idx} className="text-sm print:text-[8px] text-gray-700 flex items-start gap-2.5 print:gap-1">
                                                    <Star className="w-4 h-4 print:w-2 print:h-2 mt-0.5 shrink-0 text-[#6499E9]" />
                            ...
                            <div className="flex flex-col h-full">
                                {/* Quote Box */}
                                <div className={`p-6 print:p-2 rounded-xl border mb-6 print:mb-2 shadow-sm bg-[#F0F9FF] border-[#9EDDFF]`}>
                                    <h3 className={`text-lg print:text-[10px] font-bold mb-4 print:mb-1 border-b pb-2 print:pb-1 uppercase tracking-wide text-slate-800 border-[#9EDDFF]`}>Fiyat ve Detaylar</h3>
                                    <div className="grid grid-cols-2 gap-4 print:gap-2 mb-4 print:mb-2">
                                        <div className="bg-white p-3 print:p-1 rounded-lg border border-white/50 shadow-sm">
                                            <span className="block text-xs print:text-[7px] text-slate-400 uppercase font-bold mb-1 print:mb-0">Öğrenci</span>
                                            <div className="flex items-center gap-2">
                                                <Users className="w-5 h-5 print:w-3 print:h-3 text-[#6499E9]" />
                                                <span className="text-xl print:text-sm font-bold text-slate-800">{quote.studentCount}</span>
                                            </div>
                                        </div>

                                        {isGroupPortal && (
                                            <div className="bg-white p-3 print:p-1 rounded-lg border border-white/50 shadow-sm">
                                                <span className="block text-xs print:text-[7px] text-slate-400 uppercase font-bold mb-1 print:mb-0">Grup Lideri</span>
                                                <div className="flex items-center gap-2">
                                                    <Users className="w-5 h-5 text-[#A6F6FF] drop-shadow-sm" />
                                                    <span className="text-xl print:text-sm font-bold text-slate-800">{quote.groupLeaderCount}</span>
                                                </div>
                                            </div>
                                        )}
                                    </div>

                                    <div className={`flex flex-col gap-3 print:gap-1 border-t pt-4 print:pt-2 border-[#9EDDFF]`}>
                                        <div className="flex justify-between items-end">
                                            <div className="flex flex-col">
                                                <span className="text-slate-500 text-sm print:text-[8px] font-medium">Öğrenci Başı Ücret</span>
                                            ...
                                            <span className="text-3xl print:text-base font-black text-[#6499E9]">{formatPrice(quote.pricePerStudent)}</span>
                                        </div>
                                    ...
                                    {quote.notes && (
                                        <div className="bg-white/60 p-4 print:p-1 rounded-lg border border-white/50 text-sm print:text-[8px] text-slate-700 italic relative mt-4 print:mt-1">
                        ...
                        {/* Gallery Grid - 4 images for print */}
                        <div className="mt-auto print:mt-1">
                            <div className="grid grid-cols-3 gap-3 print:grid-cols-4 print:gap-1">
                                {program.galleryImages.slice(0, 9).map((img, idx) => (
                                    <div key={idx} className={`h-24 print:h-10 rounded-lg print:rounded overflow-hidden border shadow-sm ${theme.borderLight} ${idx >= 4 ? 'print:hidden' : ''}`}>
                    ...
                    {/* Footer Page 1 */}
                    <footer className={`w-full py-1.5 print:py-1 px-8 print:px-2 shrink-0 ${BSC_GRADIENT}`}>
                        <div className="flex justify-center items-center gap-8 print:gap-3 text-xs print:text-[7px] font-medium opacity-95 text-slate-800">
                            <div className="flex items-center gap-2 print:gap-1">
                                <Phone className="w-3 h-3 print:w-2 print:h-2" />
                                <span>+90 537 473 00 94</span>
                            </div>
                            <div className="flex items-center gap-2 print:gap-1">
                                <Mail className="w-3 h-3 print:w-2 print:h-2" />
                                <span>emre.ayan@bsceducation.com</span>
                            </div>
                            <div className="flex items-center gap-2 print:gap-1">
                                <Globe className="w-3 h-3 print:w-2 print:h-2" />
