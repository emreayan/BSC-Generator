...
const App: React.FC = () => {
  ...
  // --- File Upload Handlers (Using Compression) ---
  const handleLogoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      try {
        const compressed = await compressImage(file, 500); // Small size for logo
  ...
  // Save Logo as default
  const handleSaveLogoDefault = () => {
      if (customLogo) {
  ...
  const handleTimetableUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
        ...
        Array.from(files).forEach(async (file) => {
            try {
                // Fixed: Explicitly cast 'file' to 'File' to avoid TS error about 'unknown'
                const compressed = await compressImage(file as File, 1000);
                setTimetableImages(prev => [...prev, compressed]);
            } catch (e) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (file.type.startsWith('image/')) {
        compressImage(file, 800, 0.7)
          .then((compressed) => {
             setCurrentGalleryImages(prev => [...prev, compressed]);
          })
          .catch(e => {
